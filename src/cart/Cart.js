...
export function Cart() {
  ...
  async function qtyHandler({ type, payload }) {
    const product = cartState.cart.find((item) => item._id === payload._id);
    let qty;
    if (type === "INCREMENT") {
      qty = {
            ...
            ({
              ...
              <div className="product-card" key={_id}>
                ...
                <button
                  disabled={qty === 1}
                  onClick={() =>
                    qtyHandler({ type: "DECREMENT", payload: { _id } })
                  }
                ...
                <button
                  onClick={() =>
                    qtyHandler({ type: "INCREMENT", payload: { _id } })
                  }
                  className="btn-secondary"
                >
                  +
                ...
                {cartState.wishlist.find((item) => _id === item._id) ? (
                  <NavLink className="btn-secondary remove" to="/wishlist">
                    Go to Wishlist{" "}
                    <span className="material-icons af" style={{fontSize: "0.6rem"}}>arrow_forward_ios</span>
                  </NavLink>
                ) : (
                  <button
                    onClick={() =>
